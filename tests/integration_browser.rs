...
fn integration_browser_events() {
    ...
    // These tests only properly test the correctness, if there is no delay between
    // the simulations, so we cannot use EnigoTest here
    {
        let mut enigo = enigo::Enigo::new(&Settings::default()).unwrap();

        // Ensure mouse events don't need a sleep afterwards to work
        log::debug!("mouse location: {:?}", enigo.location().unwrap());
        enigo.move_mouse(137, 59, Abs).unwrap();
        enigo.button(Button::Left, Click).unwrap();
        assert_eq!((137, 59), enigo.location().unwrap());
